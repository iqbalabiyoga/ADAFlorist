...
?>
    ...
    <head>
        ...
        $kuery = mysqli_query($connect, "SELECT * FROM tanaman WHERE ID_Tanaman = '$id' ");
        $kueri=mysqli_fetch_array($kuery);
        if($kueri<1) echo "<script>document.location.href='tidakada.php'</script>";
        if(isset($_SESSION['admin'])){
        include 'headadmin.php';} else{
        include 'header.php';}
                            $tanaman = mysqli_fetch_array($query); $jumlih=$tanaman['jumlah_lihat']; $lihat=$tanaman['jumlah_lihat']+1; mysqli_query($connect, "update tanaman set jumlah_lihat='$lihat' where ID_Tanaman = '$id'"); ?>
    <body>
    ...
    $tots= $tanaman['Harga_Tanaman'];
            ...
            <div class="row bordered">
                ...
                $sejeniss=mysqli_query($connect,"select * from tanaman where Jenis_Tanaman = '$jenis' and ID_Tanaman!='$id' order by rand() limit 3");
                while($sejenis=mysqli_fetch_array($sejeniss)){?>
                    ...
                    $tots= $sejenis['Harga_Tanaman'];
            echo 
            number_format("$tots",0,",",".")
                    ?></strong></div><div class="col s6"><?php if ($sejenis['Jumlah_Stok']>0) echo "<h6 class='white-text teal'>Stok Tersedia</h6>"; else echo "<h6 class='white-text red'>Stok Habis</h6>";?></div></div>
                    </span>
